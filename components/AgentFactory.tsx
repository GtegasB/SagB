...
const AgentFactory: React.FC<AgentFactoryProps> = ({
    ...
    const handleSaveNewAgent = async () => {
        ...
        try {
            if (editingId) {
                // Modo Edição: Tenta atualizar documento existente
                await updateDoc(doc(db, "agents", editingId), payload);
            } else {
                // Modo Criação: Salva no Firestore
                const docRef = await addDoc(collection(db, "agents"), payload);

                // Atualiza o campo 'id' dentro do documento para ser igual ao ID gerado pelo Firestore
                await updateDoc(docRef, { id: docRef.id });

                // Atualiza o payload local para que a UI reflita o ID correto imediatamente
            ...
            if (error.code === 'resource-exhausted' || error.message.includes("quota")) {
                alert("Erro de Cota do Firebase. A imagem pode ser muito grande ou o limite diário excedido.");
            } else if (error.code === 'permission-denied') {
                alert("Erro de Permissão: Verifique as regras de segurança do Firestore.");
            } else {
                alert(`Erro ao salvar: ${error.message || error}`);
            }
            // Não limpamos o form para o usuário não perder os dados
        } finally {
            setIsSaving(false); // LIBERA BLOQUEIO
        }
    };
